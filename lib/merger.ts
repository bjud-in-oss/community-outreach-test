...
class AutonomousMergeManager {
  ...
  async mergePullRequest(url: string) {
    ...
    try {
      ...
      console.log(`PR state: ${pr.draft ? "draft" : "ready"}`);
      ...
      if (pr.draft) {
        ...
        await this.octokit.graphql(
          `mutation($pullRequestId: ID!) {
            convertPullRequestToReadyForReview(input: {pullRequestId: $pullRequestId}) {
              pullRequest {
                id
              }
            }
        ...
        console.log(`Converted draft PR #${pull_number} to ready for review.`);
      }
...
export default AutonomousMergeManager;
